...
Use ARROWS or WS keys for view control.

    UP           : Rotate camera up direction
    DOWN         : Rotate camera down direction
    LEFT         : Rotate camera left direction
    RIGHT        : Rotate camera right direction
...
from planners.PathPlanner import PathPlanner
from planners.Structures import State, Maneuver
from planners.utils import angle_between_points
...
class World(object):
    ...
    def get_waypoint(self, pos_x, pos_y):
        ...
        if (_active_maneuver == Maneuver.DECEL_TO_STOP or _active_maneuver == Maneuver.STOPPED):
    ...
    def move(self, sim_time, res=1):
        ...
        wait_time = 0.0
        delta_t = sim_time - last_move_time
        if (sim_time - last_move_time) > wait_time:
            ...
            if len(way_points) > 1:
                ...
                velocity = v_points[0]
                if velocity < 0.01 or _active_maneuver == Maneuver.STOPPED:
                    yaw = _prev_yaw
                    way_points.pop(0)
                    v_points.pop(0)
                else:
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        if _active_maneuver == Maneuver.FOLLOW_LANE:
            behavior = 'FOLLOW LANE'
        elif _active_maneuver == Maneuver.FOLLOW_VEHICLE:
            behavior = 'FOLLOW VEHICLE'
        elif _active_maneuver == Maneuver.DECEL_TO_STOP:
...
def game_loop(args):
    global update_cycle, prev_time, control_logs
    pygame.init()
    pygame.font.init()
    world = None
    vehicles_list = []
    ...
    try:
        ...
        # Logs
        control_logs = [["desired_heading", "heading", "steer_output", "desired_velocity", "velocity", "throttle_output"]]
        ...
        KP_THROTTLE, KI_THROTTLE, KD_THROTTLE = 1.0, 0.05, 0.1

        # TODO Initialize controller class
        throttle_controller = PIDController(KP_THROTTLE, KI_THROTTLE, KD_THROTTLE, output_min=-1, output_max=1)

        KP_STEER, KI_STEER, KD_STEER = 0.5, 0.1, 0.2
        # TODO Create steering PID constants.

        # TODO Initialize controller class.
        steer_controller = PIDController(KP_STEER, KI_STEER, KD_STEER, output_min=-1.2, output_max=1.2)
        ...
        with open(f"{PATH}/control_log.csv", "w") as csv_file:
            csv_writer = csv.writer(csv_file)
            csv_writer.writerows(control_logs)
...
def get_paths(path_planner : PathPlanner, data):

    if data is None:
        return None
    
    xt_points = data["traj_x"]
    yt_points = data["traj_y"]
    vt_points = data["traj_v"]
    yaw = data["yaw"]
    velocity = data["velocity"]
    sim_time = data["time"]
    waypoint_xt = data["waypoint_x"]
    waypoint_yt = data["waypoint_y"]
    waypoint_tt = data["waypoint_t"]
    is_junction = data["waypoint_j"]
    tl_state = data["tl_state"]

    have_obst = path_planner._have_obst

    if(not have_obst):
        x_obst = data["obst_x"]
        y_obst = data["obst_y"]
        path_planner.set_obst(x_obst, y_obst)

    location = carla.Location(waypoint_xt, waypoint_yt, 0)
    rotation = carla.Rotation(0, waypoint_tt, 0)
    v = carla.Vector3D()
    acceleration = carla.Vector3D()

    goal = State(location, rotation, v, acceleration)
    ...
    xt_points, yt_points, vt_points, spirals_xt, spirals_yt, spirals_vt, best_spirals_ = \
        path_planner.plan(xt_points.copy(), 
                          yt_points.copy(), 
                          vt_points.copy(), 
                          yaw, 
                          velocity, 
    ...
    msgJson = dict()
    msgJson["yaw"] = yaw
    msgJson["velocity"] = velocity
    msgJson["time"] = sim_time
    msgJson["trajectory_x"] = xt_points
    msgJson["trajectory_y"] = yt_points
    msgJson["trajectory_v"] = vt_points
    msgJson["location_x"] = data["location_x"]
    msgJson["location_y"] = data["location_y"]
    msgJson["location_z"] = data["location_z"]
    msgJson["spirals_x"] = spirals_xt
    msgJson["spirals_y"] = spirals_yt
    msgJson["spirals_v"] = spirals_vt
    msgJson["spiral_idx"] = best_spirals_
    msgJson["active_maneuver"] = path_planner.get_active_maneuver()
...
def get_control(steer_controller, throttle_controller, data):
    global prev_time, control_logs

    xt_points = data["trajectory_x"]
    yt_points = data["trajectory_y"]
    vt_points = data["trajectory_v"]
    yaw = data["yaw"]

    velocity = data["velocity"]
    x_position = data["location_x"]
    y_position = data["location_y"]
    z_position = data["location_z"]

    current_time = data["time"]

    dt = current_time - prev_time
    if dt <= 0:
        dt = 0.001  # Ensure dt is never zero to avoid division by zero
    ...
    ###### Steering Control #############
    #####################################
    if xt_points and yt_points:  # Check that lists are not empty
        desired_heading = angle_between_points(x_position, y_position, xt_points[0], yt_points[0])
        steer_error = desired_heading - yaw
        steer_command = steer_controller.get_control_command(steer_error, dt)
    else:
        print("Error: xt_points or yt_points is empty.")
        desired_heading = 0  # Default value if lists are empty
        steer_command = 0  # Default or maintain previous command

    #####################################
    ###### Throttle Control #############
    #####################################
    desired_v = vt_points[0] if vt_points else 0
    throttle_error = desired_v - velocity
    throttle_command = throttle_controller.get_control_command(throttle_error, dt)

    # Decide on throttle or brake command based on the sign of throttle_command
    if throttle_command > 0.0:
        brake_command = 0.0
    else:
        brake_command = -throttle_command
        throttle_command = 0.0

    control_logs.append([desired_heading, yaw, steer_command, desired_v, velocity, throttle_command])

    # Update the data dictionary with new values
    data["brake"] = brake_command
    data["throttle"] = throttle_command
    data["steer"] = steer_command
